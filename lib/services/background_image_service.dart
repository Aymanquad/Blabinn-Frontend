...
class BackgroundImageService {
  ...
  Future<void> handleReceivedImageMessage(Message message,
    ...
    try {
      ...
      if (senderName == null) {
        try {
          final friendProfile =
              await _apiService.getUserProfile(message.senderId);
          friendName = friendProfile['firstName'] ??
  ...
  Future<void> _downloadAndSaveImage(Message message, String senderName) async {
    try {
      ...
      final tempFileName =
          '${DateTime.now().millisecondsSinceEpoch}_received.jpg';
  ...
  Future<void> handleImageFromPushNotification(
  ...
  static Future<bool> isImageAlreadyReceived(
      {required String imageUrl, String? messageId}) async {
    try {
      final directory = await getApplicationDocumentsDirectory();
