...
class _RandomChatScreenState extends State<RandomChatScreen> {
  ...
  void _validateSession() {
    ...
    print('   🤖 Is AI Chat: ${widget.isAiChat}');
  }

  void _setupAiChat() async {
    print('🤖 [AI CHAT] Setting up AI chat');

    // Set partner info from AI user data
    if (widget.aiUser != null) {
      _partnerInfo = widget.aiUser;
      print('🤖 [AI CHAT] AI user info loaded: ${_partnerInfo?['username']}');
    }

    // Create backend AI session for intelligent responses
    try {
      final currentUser = FirebaseAuth.FirebaseAuth.instance.currentUser;
      if (currentUser != null) {
        print('🤖 [AI CHAT] Creating backend AI session...');
  ...
  void _initializeServices() {
    ...
    // Only initialize socket service for human chats
    if (!widget.isAiChat) {
      _socketService = SocketService();
      print('📡 [RANDOM CHAT DEBUG] Socket service initialized for human chat');
    } else {
      print(
          '🤖 [RANDOM CHAT DEBUG] Skipping socket service initialization for AI chat');
    }
  }
  ...
  void _setupSocketListeners() {
    if (_socketService == null) {
      print(
          '📡 [RANDOM CHAT DEBUG] Socket service not initialized for AI chat');
      return;
    }
  ...
  void _joinChatRoom() {
    if (_socketService == null) {
      print('🔌 [RANDOM CHAT DEBUG] Socket service not available for AI chat');
      return;
    }
    ...
    try {
      ...
      if (!_socketService!.isConnected) {
        print('🔌 [RANDOM CHAT DEBUG] Waiting for socket connection...');
        Timer.periodic(const Duration(milliseconds: 200), (timer) {
          if (_socketService!.isConnected) {
            timer.cancel();
            _socketService!.joinChat(widget.chatRoomId);
            print(
                '🔌 [RANDOM CHAT DEBUG] Joined chat room: ${widget.chatRoomId}');
          } else if (timer.tick > 25) {
            // 5 seconds timeout
            timer.cancel();
            print(
                '❌ [RANDOM CHAT DEBUG] Timeout waiting for socket connection');
  ...
  void _cleanupListeners() {
    // Only cleanup socket subscriptions if they exist (for human chats)
    if (_messageSubscription != null) {
      try {
        ...
        print(
            '⚠️ [RANDOM CHAT DEBUG] Error cancelling message subscription: $e');
      }
    }
  ...
  Future<void> _leaveChatRoom() async {
    if (!_isSessionActive || _socketService == null) return;
  ...
  Future<void> _stopRandomConnection() async {
    ...
    try {
      ...
      await _socketService!.stopRandomConnection(userId: currentUserId);
      print('[RANDOM CHAT DEBUG] Stopped random connection');
    } catch (e) {
      print('[RANDOM CHAT DEBUG] Error stopping random connection: $e');
    }
  }
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      ...
      print(
          '🤖 [AI CHAT DEBUG] _sendMessage called. widget.isAiChat: ${widget.isAiChat}');

      if (widget.isAiChat) {
        // Send message to AI chatbot
        print('🤖 [AI CHAT] Sending message to AI chatbot...');
        await _sendAiMessage(processedContent);
      } else {
        // Send processed message via socket
        print('📡 [SOCKET CHAT] Sending message via WebSocket...');
        if (_socketService != null) {
  ...
  Future<void> _sendAiMessage(String content) async {
    try {
      print('🤖 [AI CHAT] Sending message to AI: $content');
      print('🤖 [AI CHAT DEBUG] Session ID: ${widget.sessionId}');
      ...
      print('🤖 [AI CHAT DEBUG] User ID: ${currentUser.uid}');
      print('🤖 [AI CHAT DEBUG] Backend Session ID: $_backendSessionId');

      // Use backend AI if session exists, otherwise use local
      if (_backendSessionId != null) {
        // Send message to backend chatbot session
        final response = await _apiService.postJson(
          '/chatbot/session/$_backendSessionId/message',
          {
            'message': content,
          },
        );
        ...
        // No backend session, use local responses
        print('💬 [CHAT] No backend session, using local responses');
        await _generateLocalAiResponse(content);
        return;
      }
    } catch (e) {
  ...
  Future<void> _endSession(String reason) async {
    ...
    if (widget.isAiChat) {
      ...
      try {
        ...
        print('❌ [AI CHAT] Error ending AI session: $e');
      }
    } else {
      ...
      try {
        ...
        if (_socketService != null) {
          ...
          print(
              '⚠️ [RANDOM CHAT DEBUG] Socket service not available, skipping socket end');
        }
      } catch (e) {
        ...
        try {
          ...
          print(
              '❌ [RANDOM CHAT DEBUG] Error ending session via API: $apiError');
        }
      }
    }
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: ClipOval(
                            child: _partnerInfo!['profilePictureUrl'] != null
                                ? Image.network(
                                    _partnerInfo!['profilePictureUrl']
                                        as String,
                                    fit: BoxFit.cover,
                                    errorBuilder: (context, error, stackTrace) {
                                      return Container(
                              ...
                              children: [
                                TextField(
                                  ...
                                  decoration: InputDecoration(
                                    ...
                                    suffixIcon:
                                        _messageController.text.length > 900
                                            ? Icon(
                                                Icons.warning,
