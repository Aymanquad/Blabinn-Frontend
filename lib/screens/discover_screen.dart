...
class _DiscoverScreenState extends State<DiscoverScreen> {
  ...
  Future<void> _likeUser(User user) async {
    try {
      final result = await _matchingService.likeUser(user.uid, message: "I liked your profile!");
  ...
  Future<void> _superLikeUser(User user) async {
    ...
    if (!_matchingService.canUseSuperLike(currentUser)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Super likes not available for your user type or limit reached'),
    ...
    try {
      final result = await _matchingService.superLikeUser(user.uid, message: "Super like! ⭐");
      
      if (result['success'] == true) {
        // Show success message
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Super liked ${user.displayName ?? user.username}! ⭐'),
  ...
  Future<void> _startInstantMatch() async {
    try {
      ...
      if (result['success'] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to start instant match: ${result['message']}'),
            backgroundColor: Colors.red,
          ),
        );
      }
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      _matchingService.getUserTypeExplanation(currentUser.userType),
            ...
            Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Consumer<UserProvider>(
                    builder: (context, userProvider, child) {
                      ...
                      return PremiumFeatureWidget(
                        ...
                        showUpgradeButton: false,
                        child: IconButton(
                          onPressed: canSuperLike ? () => _superLikeUser(user) : null,
                  ...
                  // Next Button
                  IconButton(
                    onPressed: _currentIndex < _potentialMatches.length - 1 ? _nextUser : null,
